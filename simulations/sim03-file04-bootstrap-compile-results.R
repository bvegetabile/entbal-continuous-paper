...
bsdata <- readRDS('output/sim03-bootstrap-results.rds')
...
simres <- simrun_lite(1, n_obs = 200,
...
bstime <- system.time(bsres <- foreach(
  i = 1:NB,
  .combine = rbind,
  .verbose = F) %dopar% bsest_smalln(i, simres$dat, A_test, nm = 3)
)
stopCluster(cl)
print(bstime)
...
pdf('paper-figures/smalln/smalln-bs-plot-one.pdf', height = 6, width = 12)
par(mfrow=c(2,2), oma = c(0,0,0,0), mar = c(4.25,4,2,1))
plot(A_test, bsdata$truth, type = 'l', lwd = 3, col = rgb(0.75,0,0,0.75), ylim = c(-5, 5),
     ylab = 'Outcome', xlab = 'Exposure Level',
