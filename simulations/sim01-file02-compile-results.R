...
betastab_mean <- t(apply(abs(betas),2,rowMeans))
betastab_max <- t(apply(abs(betas),2,function(x) apply(x, 1, max)))
...
truth50 <- - (A50 + 5) * (A50 - 5) / 300 + A50 * (2 + MX1^2 + MX2^2) / 25 + MX1 + MX2 + MX3
...
pdf('paper-figures/eb-response-loess.pdf', height = 4, width = 12)
...
baltab2 <- cbind(betastab_max, cortab_max, kstab_max)
